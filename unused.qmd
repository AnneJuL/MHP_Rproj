...
## Lion map sized by num of lions

```{r}
# Add lion points in red, sized by number of lions
lion_sized_mapleaflet <- CD_peryear_mapleaflet |>
...
lion_sized_mapleaflet
```

## Random points for year of creation of the communal dens :
...
### 1.4.1. tblLifeHistory - NOT USE ANYMORE

Then, with the IDs and the year of birth, I will match the `id` column from `CD_monthly_data` to the IDs in the new dataframe. If the ID is found in the hyena column, then it is a CD and we keep it.
...
### 1.2.1. tblSession
```{r}
# This is with tlbSessions, we want to use tblHyenaPerSession:
