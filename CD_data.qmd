...
DB_locations_for_MCPs_clean <- DB_locations_for_MCPs |>
  ...
  mutate(
    ...
    utmn = if_else(landmark_id == "Stupid_Tree" & utme == 0 & utmn == 0,
                   utmn_new, utmn)) |>
   mutate(
    utme = if_else(landmark_id == "Rhino_Tree" & utme == 0 & utmn == 0,
                   utme_new, utme),
    utmn = if_else(landmark_id == "Rhino_Tree" & utme == 0 & utmn == 0,
                   utmn_new, utmn)) |>
   mutate(
    utme = if_else(landmark_id == "Saga_Lugga_Crossing" & utme == 0 & utmn == 0,
                   utme_new, utme),
    utmn = if_else(landmark_id == "Saga_Lugga_Crossing" & utme == 0 & utmn == 0,
                   utmn_new, utmn)) |>
...
# i have 183 rows with weird coordinates, we will examine them...
DF_check <- DB_locations_for_KDEs_clean |>
  filter(nchar(utme) != 6 | nchar(utmn) != 7)

# for now, i will just take them out, to be able to visualize the points, but we will figure out what to do with it 

DB_locations_for_KDEs_test <- DB_locations_for_KDEs_clean |> #DB_locations_for_KDEs_clean = 71751 rows
  filter(!session %in% DF_check$session)
...
DB_locations_for_KDEs_test_sf <- st_as_sf(DB_locations_for_KDEs_test, coords = c("utme", "utmn"), crs = 32736)

# map to visualize
mapview(DB_locations_for_KDEs_test_sf, xcol = "utme", ycol = "utmn", zcol = "category")
